# Imports the basic data processing libraries
import pandas as pd
import numpy as np
# Imports RegEx for preproc text
import re 
# Imports Streamlit for the web app creation
...
# Imports the Altair library for Streamlit data visualization + charts
...
def combineFeaturesForSingleTextDF(single_text_df, scaler, feature_cols):
    ...
    # Gets the max. value from engineered features to check if it is a val greater than 1, to see if scaled properly
...
def explainPredictionWithLIME(
    ...
    - It uses LIME to generate local explanations (word and feature imporances) for the individual prediction.
        ...
        fasttext_model (<fasttext.FastText._FastText> model): a supervised, pre-trained FastText model for extracting 
                                                                  dense text embeddings, trained on the training data from
                                                                  four domain-specific datasets
                                                    
        classifier_model (sklearn.calibration.CalibratedClassifierCV): the pre-trained Calibrated Classifier which is
                                                                    wrapping a Passive-Aggressive base classifier for 
                                                                    outputting probabilities

        fitted_scaler (sklearn.preprocessing._data.StandardScaler): a pre-trained scikit-learn StandardScaler, fitted on the same 
                                                                 combined dataset, for scaling the extra features

        text (str): the text to output the fake or real news prediction for
        ...
        num_features (int): the number of top word features LIME should output the importance scores for

        num_perturbed_samples (int): the number of perturbed samples to use for LIME explanations
    ...
    def predict_fn(perturbed_texts):
        ...
        # Returns probs of permuted samples to LIME explainer to generate explanations based on prob differences to original text
    ...
    """"
        LIME Documentation: "First, we generate neighborhood data by randomly perturbing features from the instance [...]
    ...
    explanation = text_explainer.explain_instance(
        ...
        predict_fn,
        num_features=num_features, # Number of top/most important words to output LIME importance scores for
        num_samples=num_perturbed_samples, # Number of perturbed versions of the text to generate; the more the greater accuracy, but takes more time
...
def highlightText(text, word_feature_list, text_prediction):
    ...
    for word_feature, importance in word_feature_list:
        ...
        pos = 0 # Starts at the first character in the text

        # Breaks out of the loop when no more of this curernt word feature are found in the text
        while True:

            # Finds the next occurence of important word (in lowercase) in the remaining text from "pos" onwards
            pos = text.lower().find(word_feature.lower(), pos)
            
            # If the word was not found in remaining text, break out of while loop
            if pos == -1:
                break
                
            # Checks if the word feature found is a valid word, not a sub-word of a different word
            ...
            # If the word feature is just a subword, increment pos and move to next part of the text string, restart at beginning of while loop
            if boundary_positions is None:
                pos += 1 
                continue
            
            # If the word feature is a whole word, unpack its beginning and end pos (character indices)
            word_start_pos, word_end_pos = boundary_positions

            # Maps the red (fake) and blue (real) colors to which class the feature is pushing the classifier towards
...
def displayAnalysisResults(explanation_dict, container, news_text, feature_extractor, FEATURE_EXPLANATIONS):
    ...
    # Defines the color and boldness with markdown and HTML depending on the predicted label
    ...
    # Adds more padding between the two bar charts
    st.markdown("""
                    <style>
                        .stColumn > div {
                            padding: 0 20px;
    ...
    # First column: word feature importance charts
    with col1:
        ...
        # Calculates the maximum importance value for pro-prediction word features and opposing-prediction word features
        ...
        if len(main_prediction_top_word_features_df) > 0:
            max_importance_score = max(max_importance_score, main_prediction_top_word_features_df["Importance"].max()) # Updates maximum score if word importance max greater than 0
            ...
            word_features_chart = alt.Chart(main_prediction_top_word_features_df).mark_bar().encode( 
                ...
                x=alt.X(
                    ...
                    axis=alt.Axis(
                        ...
                        labelOverlap=False  # Prevents overlapping labels
                    )),
        ...
        if len(opposite_prediction_top_word_features_df) > 0:
            ...
            opposite_word_features_chart = alt.Chart(opposite_df_for_chart).mark_bar().encode(
                x=alt.X(
                    ...
                    title="Word Feature",
                    axis=alt.Axis(
                        labelAngle=-45, # Rotates the labels for improved readability
                        labelLimit=150, # Sets the label sizes to a maximum 150 pixels
                        labelOverlap=False # Do not let labels overlap
                    )),
    ...
    with col2:
        ...
        with col2.expander("*View More Detailed Feature Score Information*"):
            ...
            for index, row in extra_features_df.iterrows():
                ...
                # Determines the importance color and explanation strings based on the prediction and score value
                if row["Importance"] > 0: # If the same direction of word importance as the main prediction
                ...
                elif row["Importance"] < 0: # Different direction of word importance as main pred
                    if main_prediction == 0: # Main pred is real BUTthe word pushes towards fake news
                        importance_color = "red"
                        importance_explanation = "(pushing towards fake news)"
                    else: # Main pred is fake BUT this word pushes towards real news
                        importance_color = "dodgerblue"
                        importance_explanation =  "(pushing towards real news)"    
                else:
                ...
                # Adds explanations from FEATURE_EXPLANATIONS about the feature and main patterns associated with it in the training data
                container.markdown(f"""
                **{row["Feature"]}**  
