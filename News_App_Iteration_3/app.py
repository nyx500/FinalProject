# Main app starting point is this file, app.,py

# Imports the required libraries
# Streamlit for creating the actual web app
import streamlit as st
# Sets the Streamlit page configuration e.g. page title and centered layout for one big col
...
# Imports the newspaper3k library for scraping the news articles from user-inputted URLs
from newspaper import Article
# Imports matplotlib for loading in the graphs and charts showing global fake vs real news patterns
import matplotlib.pyplot as plt
# Imports the library for model loading 
import joblib
# Imports for downloading the FastText model from Google Drive
import gdown
import fasttext
# Imports the custom feature extractor class
from feature_extractor import BasicFeatureExtractor
# Imports the custom explainability functions from lime_functions.py separate module for generating LIME explanations
from lime_functions import explainPredictionWithLIME, displayAnalysisResults

# Creates a mapping of the DataFrame's extra feature column names to explanations about their tendencies and patterns in training data
FEATURE_EXPLANATIONS = {
    "exclamation_point_frequency": 
        """Exclamation mark frequency count. Higher raw ! counts in the training data
        signal more emotional or sensational writing, and were more strongly associated 
        with fake news.""",
    ...
    "noun_to_verb_ratio": 
        """Ratio of nouns to verbs. Higher ratios, i.e. more nouns, suggest more background knowledge based, factual rather than action-focused writing. 
    ...
    "person_named_entity_frequency": 
        ...
        associated with fake news, showing that disinformation campaigns (at least based on this specific training data),
    ...
    "flesch_kincaid_readability_score": 
        """U.S. grade level required to understand the text. A higher raw score represents more complex language used, which was 
...
# Adds another warning msg for the first 2 (URL and text input) tabs
...
def load_fasttext_model():
    # Loads the pre-trained fastText from Google Drive
    url = "https://drive.google.com/uc?id=1uO8GwNHb4IhqR2RNZqp1K-FdmX6FPnDQ"
    local_path =  "/tmp/fasttext_model.bin" # Saves it to this path
    gdown.download(url, local_path, quiet=False)
    return fasttext.load_model(local_path)

# Loads the CalibratedClassifierCV model (wrapping the Passive-Aggressive Classifier to output probabilities), trained on the 4 unimodal datasets combined
@st.cache_resource 
def load_pipeline():
...
# Loads the models and shows the loading progress to users with a spinner
...
# Creates the tabs for news text classification, visualizations, app about information
tabs = st.tabs(["Enter News as URL", "Paste in Text Directly", "Key Pattern Visualizations",
                "Word Clouds: Real vs Fake", "How it Works..."])

# Adds some custom CSS that makes the main app container slightly wider, to stop all content being all cramped together
...
with st.container():
    ...
    # First tab: news is inputted by user as URL
    with tabs[0]:
        ...
        num_perturbed_samples = st.slider(
            ...
            min_value=25,
            max_value=500,
            value=50,  # Default value is 50 perturbations
            step=25, # Adds a step size of 25
            help="Increasing the number of samples will make the outputted explanations more accurate but may take longer to process!"
        )
        
        # Explains the slider and how feature importance works to users
        st.markdown("The more perturbed samples you choose, the more accurate the explanation will be, but it will take longer to output.")
        ...
        if st.button("Classify", key="classify_button_url"): # If URL classificationbutton pressed...
            ...
            # Checks if news URL input not empty if button is pressed
            if url.strip():  
                # Tries to extract the news text from the URL using the newspaper3k library (this can sometimes fail, so adds a try-except block here)
                try:

                    # Displays the progress of extracting news
    ...
    with tabs[1]:
        ...
        st.markdown("The more perturbed samples you choose, the more accurate the explanation will be, but it will take longer to compute.")
                ...
                try:
                    ...
                    with st.spinner(f"Analyzing text with {num_perturbed_samples} perturbed samples..."):
                        explanation_dict = explainPredictionWithLIME(
                            ...
                            pipeline, # PAC model wrapped in CalibratedClassifierCV
                            scaler, # Pre-fitted StandardScaler
                            news_text, # The user's inputted news text
                            feature_extractor, # Feature extractor instance for engineered features
                            num_features=50, # Number of word features LIME generates with importance scores
                            num_perturbed_samples=num_perturbed_samples # User-inputted number of perturbed samples for LIME to generate
                        )
    ...
    with tabs[2]:
        ...
        # Adds titles and subtitle
        st.header("Key Patterns in the Training Dataset: Real (Blue) vs Fake (Red) News")

        st.markdown("These visualizations show the main trends and patterns between real and fake news articles in the training data.")
        ...
        st.markdown("In the training data, real news tended to have slightly more nouns than verbs than fake news.")

        st.markdown("<br>", unsafe_allow_html=True)
        
        # Emotion score NRC lexicon counts
        ...
        st.markdown("Fake news (in this dataset) often contained more references to PERSON named entities than real news.")
        ...
        st.markdown("Fake news tended to contain less numerical data (i.e. lower CARDINAL named entity frequencies) than real news.")
        ...
        st.markdown("Real news tended to have a slightly higher U.S. grade level, indicating more complex language, than fake news.")
        ...
        st.markdown("Real news tended to contain more complex words than fake news.")
        ...
        with st.expander("📊 Details about these visualizations"):
            ...
            raw frequencies have been divided by the text length (in words) to account for the differences in the lengths of all the news texts.
            
            ### Some of the Main Differences between Real News and Fake News Based on the Data Analysis:
            
            - **Capital Letter Frequencies:** Higher frequencies were found in real news, perhaps due to the greater usage of proper nouns and techical acronyms in legitimate news
            - **Third-person Pronoun Frequencies:** Third-person pronouns were more frequenty encountered in fake news in these datasets, suggesting a storytelling-like narrative style and person-focused content in fake news
    ...
    with tabs[3]:
        ...
        st.markdown("These word clouds visualize the most frequent named entities (e.g. people, organizations, countries) in real vs fake news articles in the training data. The size of each word is proportional to how frequently it appears.")
        ...
        # Adds an explanation for how the word clouds use sizes colors
