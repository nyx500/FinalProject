...
# Imports the data processing libraries
import pandas as pd
import numpy as np
import re
# Imports a model-loading library
import joblib
# Imports the LIME-explainability library
import lime.lime_text
# Imports matplotlib for visualizations
import matplotlib.pyplot as plt

# Loads the WELFake-trained Passive-Aggressive classifier model using st.cache_resource
...
def generateLIMEExplanation(pipeline, text, num_features=30):
    """
        Generates LIME-based importance scores for individual word features for a single news text. 
        Displays a chart showing the top important features pushing the classifier towards a specific prediction.
        ...
        Output:
            fig (matplotlib.pyplot.figure): the feature importance bar chart to show in the app
    ...
    # Stores the extracted most important features as a list of (word, importance) tuples
    top_features = explanation.as_list()

    # Creates a pandas DataFrame with the word features and their corresponding LIME scores in two columns
    df = pd.DataFrame(top_features, columns=["Word", "Importance Score"])

    # Sorts the DataFrame by the absolute value ("Absolute Score") of the importance scores: first create a new column storing these absolute scores
    df["Absolute Score"] = df["Importance Score"].apply(lambda x: abs(x))
    
    # Uses the pandas .sort_values function to sort the DataFrame's rows in descending (ascending=False) order using this new absolute score column
    df_sorted = df.sort_values(by="Absolute Score", ascending=False)

    # Now extracts the sorted word features and their scores for plotting
    sorted_words = df_sorted["Word"].tolist()
    sorted_scores = df_sorted["Importance Score"].tolist()

    # Plots the sorted columns as a bar chart using matplotlib: importance scores pushing classifier towards Real prediction in blue, towards Fake in red
    fig, ax = plt.subplots(figsize=(10, 6))
    
    # Separates the pushing-towards-positive class (1 = Fake) and pushing-towards-negative class (0 = Real) importancescores into red and blue colors
    colors = ["red" if score > 0 else "blue" for score in sorted_scores]
    
    # Creates a range of integers from 0 to the number of most important words for plotting the y-axis
    y_pos = np.arange(len(sorted_words))
    
    # Creates horizontal bar-chart with words on the y-axis and the importance (scores) constituting the width. Also maps each word to the prediction color
    ax.barh(y_pos, sorted_scores, color=colors)
    # Arranges the word positions on the y-axis
    ax.set_yticks(y_pos)
    # Labels the words on the y-axis
    ax.set_yticklabels(sorted_words)
    # Labels the x-axis showing word importance score
    ax.set_xlabel("Word Score (Red = Fake, Blue = Real)")
    # Adds thetitle
    ax.set_title("LIME Explanation (Sorted by Importance)")
    
    # Gets the current axes (matplotlib plot) and invert the y-axis to show the most important words at the top
    # Reference: https://stackoverflow.com/questions/2051744/how-to-invert-the-x-or-y-axis
    plt.gca().invert_yaxis()
    
    # Returns the matplotlib figure object and the list of LIME importance (word, score) tuples
...
def convertToNaturalLanguageExplanation(lime_features, prob):
  ...
  # Converts the probability score for the text being fake news into a percentage to the nearest integer and round the percentage to an integer
  perc = round(prob * 100)

  # If the probability is over 0.5, then show overall prediction as fake
  if prob > 0.5:
    category = "fake"
  # If the probability is less than 0.5, then show overall prediction as real
  elif prob < 0.5:
    category = "real"
  # If the probability is 0, set category to "neither fake nor real"
  else:
    category = "neither fake nor real"

  # Creates a natural language explanation describing what the probability score means
  ...
  # Iterates over the most important features to generate the natural language explanation
  for feature_tuple in lime_features:
    # Adds the word features based on certain conditions: only add the word feature for explanation if its importance score is actually pushing
    ...
    # Only includes the top 5 features for the natural language explanation
    if included_features == 5:
        break

  # Writes the generate the natural language explanation about the word features and what their importance means
  ...
  # Concatenates the natural language descriptions
  return probability_explanation + feature_explanation

    
######################################################### Streamlit Application ####################################################################################

# Adds the title to the top of the application
...
with tab1:
    ...
    # Creates a dropdown for selecting analysis options: either LIME-based charts or natural-language based text explanation of the important featurs
    explanation_type = st.selectbox(
        "Choose explanation type:",
        ["LIME Explainer", "Natural Language"]
    )

    # When the user clicks on the "Analyze Text" button
        ...
        if news_text:
            try:
                ...
                if explanation_type == "LIME Explainer":
                    ...
                    # Displays the matplotlib figure bar chart showing color-coded importance scores
                    # Reference: https://docs.streamlit.io/develop/api-reference/charts/st.pyplot
                    st.pyplot(figure)      

                    # Displays the top features as a bulletted list using the Streamlit Markdown function
                    st.markdown("## Top 10 Ranked Words Influencing Prediction:")

                    # Shows the list of important words
                    ranked_words = ""
                    for feature, score in features[:10]:
                        ...
                        if score == 0:
                            news_label = "neutral"
                        # Shows the importance score to 3 decimal points
